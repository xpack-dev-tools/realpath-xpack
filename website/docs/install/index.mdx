...
The **xPack GNU realpath** can be installed automatically, using the `xpm` command
(the recommended method), or manually, by downloading and unpacking one of the
portable archives.

## Automated install

The easiest (and recommended) way to install GNU realpath
is by using the **binary xPack**, available
as <a href={ `https://www.npmjs.com/package/@xpack-dev-tools/realpath` }><code>@xpack-dev-tools/realpath</code></a> from
...
<code>xpacks/@xpack-dev-tools/realpath</code>.
...
**realpath** package into the project:
...
* identify the platform specific archive for the latest available version
  of GNU realpath, download it into a cache and unpack
...
* create a local symbolic link
  like <code>xpacks/@xpack-dev-tools/realpath</code> pointing to the
...
* add links/forwarders into
  ...
  <code>xpacks/@xpack-dev-tools/realpath/.content/bin</code>;
* add <code>@xpack-dev-tools/realpath</code> to
  `package.json` as a
  development dependency; this associates a specific version of GNU realpath with
...
it is also possible to install GNU realpath only globally in
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/realpath@latest --global --verbose`
} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/AppData/Roaming/xPacks/\@pack-dev-tools/realpath /${ customField('upstreamVersion') }-${ customField('xpackSubversion') }.${ customField('npmSubversion') }/.content/bin:$PATH`
} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/realpath /${ customField('upstreamVersion') }-${ customField('xpackSubversion') }.${ customField('npmSubversion') }/.content/bin:$PATH`
} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/realpath /${ customField('upstreamVersion') }-${ customField('xpackSubversion') }.${ customField('npmSubversion') }/.content/bin:$PATH`
} </CodeBlock>
...
<CodeBlock language="sh"> {
`xpm uninstall @xpack-dev-tools/realpath --verbose`
} </CodeBlock>
...
the <a href={ `https://github.com/xpack-dev-tools/realpath-xpack/releases/` }>GitHub Releases</a> pages.
...
The Windows versions of **xPack GNU realpath**
are packed as `.zip` files.
Download the latest version named like:

* <code>xpack-realpath-{ customField('upstreamVersion') }-{ customField('xpackSubversion') }-win32-x64.zip</code>
...
To manually install the <b>xPack GNU realpath</b>,
unpack the archive and move it to a location of your choice.

The recommended location is
the <code>%USERPROFILE%\\AppData\\Roaming\\xPacks\\realpath</code> folder, for
example <code>C:\\Users\\ilg\\AppData\\Roaming\\xPacks\\realpath\\xpack-realpath-{ customField('upstreamVersion') }-{ customField('xpackSubversion') }</code>.
...
* <code>xpack-realpath-{ customField('upstreamVersion') }-{ customField('xpackSubversion') }-darwin-x64.tar.gz</code>
* <code>xpack-realpath-{ customField('upstreamVersion') }-{ customField('xpackSubversion') }-darwin-arm64.tar.gz</code>
...
example <code>/Users/ilg/Library/xPacks/realpath/xpack-realpath-{ customField('upstreamVersion') }-{ customField('xpackSubversion') }</code>:

<CodeBlock language="console"> {
`mkdir -p ~/Library/xPacks/realpath
cd ~/Library/xPacks/realpath

tar xvf ~/Downloads/xpack-realpath-${ customField('upstreamVersion') }-${ customField('xpackSubversion') }-darwin-x64.tar.gz
chmod -R -w xpack-realpath-${ customField('upstreamVersion') }-${ customField('xpackSubversion') }`
} </CodeBlock>
...
example <code>/home/ilg/.local/xPacks/realpath/xpack-realpath-{ customField('upstreamVersion') }-{ customField('xpackSubversion') }</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/realpath
cd ~/.local/xPacks/realpath

tar xvf ~/Downloads/xpack-realpath-${ customField('upstreamVersion') }-${ customField('xpackSubversion') }-linux-x64.tar.gz
chmod -R -w xpack-realpath-${ customField('upstreamVersion') }-${ customField('xpackSubversion') }`
} </CodeBlock>
