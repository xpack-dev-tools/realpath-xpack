...
The easiest (and recommended) way to install xPack GNU realpath
...
* add <code>@xpack-dev-tools/realpath</code> to
  `package.json` as a
  development dependency; this associates a specific version of xPack GNU realpath with
...
it is also possible to install xPack GNU realpath only globally in
...
In order to access the xPack GNU realpath binaries installed in
