...
**@xpack-dev-tools/realpath** is an xpm package that packages platform-specific archives, built with the [xPack Build Box (XBB)](https://xpack.github.io/xpack-build-box/).
...
```sh
rm -rf ~/Work/xpack-dev-tools/realpath-xpack.git/package-lock.json
```

```sh
rm -rf ~/Work/xpack-dev-tools/realpath-xpack.git/build-assets/package-lock.json
git -C ~/Work/xpack-dev-tools/realpath-xpack.git pull

npm --prefix ~/Work/xpack-dev-tools/realpath-xpack.git/build-assets install
xpm install -C ~/Work/xpack-dev-tools/realpath-xpack.git/build-assets
...
(such as **9.8.0-1.1**) with the following expression:
...
xpm run trigger-workflow-test-docker-linux-arm64 -C ~/Work/xpack-dev-tools/realpath-xpack.git/build-assets
```
...
tagged such as **v9.8.0-1** (mind the dash in the middle!) and
named such as **xPack GNU realpath v9.8.0-1** (mind the dash),
...
- **Commit** all changes with the message
  _**package.json: update URLs for 9.8.0-1.1 release**_
...
- Navigate to the [GitHub Releases](https://github.com/xpack-dev-tools/realpath-xpack/releases) page
...
npm --prefix ~/Work/xpack-dev-tools/realpath-xpack.git/build-assets install && \
xpm install -C ~/Work/xpack-dev-tools/realpath-xpack.git/build-assets && \
xpm run test-release -C ~/Work/xpack-dev-tools/realpath-xpack.git/build-assets
```
